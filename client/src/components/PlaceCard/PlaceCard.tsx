...
const PlaceCard: React.FC<Props> = ({ restaurant }: Restaurant) => {
  ...
  return (
    <TinderCard ref = {tinderCardRef} key={restaurant.displayName && restaurant.displayName.text} onSwipe={(dir) => swiped(dir, restaurant)} onCardLeftScreen={() => outOfFrame(restaurant.name)}>
      {
        dontShow ? 
